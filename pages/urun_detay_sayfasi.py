...
class UrunDetaySayfasi(PageBase):
    ...
    def __init__(self, driver): # test classlarındaki driver çalıştırmak için
        super().__init__(driver)#pagebase deki driver çalıştırmak için
        self.driver = driver

    SEPETTEKI_URUN_YAZISI = (By.CSS_SELECTOR, "a.ico-cart span:nth-child(2)") #hard code olmaktan çıkarıyoruz ufak bir degişiklikte
    #burda yapılınca tek tek aşagıdaki fonksiyonlara gitmeye gerek kalmayacak
    ...
    def add_to_cart_buttona_tikla(self):
        add_to_cart_button = self.wait_element_visibility(*UrunDetaySayfasi.ADD_TO_CART_BUTTON)#pagebase yani parent classdan geliyor
